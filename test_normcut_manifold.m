...
P = generateRandomPointsOnCylinder(8001)+randn(8001,3)*0.02;
P(:,3) =P(:,3)*3;
P(:,2) =P(:,2)*0.69;


P2 = generateRandomPointsOnCube(3001)+randn(3001,3)*0.02;
P2(:,3) =P2(:,3)*2.8+6.17;
P2(:,2) =P2(:,2)*2 ;

P3 = -1 + 2*rand(ceil(500), 3);     % [-1,1]^3에서 균일 샘플
%P3 = P3(sum(P3.^2, 2) <= 1, :); 
P3(:,3) = P3(:,3)*1;
P3(:,2) = P3(:,2)*1;

P4 = generateRandomPointsOnCube(2401)+randn(2401,3)*0.01;
P4(:,3) =P4(:,3)*1+6.17;
P4(:,2) =P4(:,2)*2.6 + 4.8 ;

%P_use = [P1(1:10:end,:) ; P2(1:10:end,:);P3(1:10:end,:) ; P4(1:10:end,:)];
%P_use = [Pi(1:20:end,:)];
P_use = [P(1:end,:) ; P2(1:end,:);P3(1:end,:)];
P_use = [P_use; P4];
...
plot(2:60, lambda_vals(2:60).' ./ ((2:60)-1).^1, 'o-')
xlabel('k');
ylabel('\lambda_k / k');
title('L_{sym}: \lambda_k/k (k=2..7)');
grid on; box on; 
%%
cluster = 1; targetcut = 3;
...
%양수 음수로 구분
margin = 0.0006;  % 예: 0.01 ~ 0.02 사이로 조정
...
axis equal
xlim([-5.5 5.5]); ylim([-4.5 4.5]); zlim([-4.5 4.5]);   % 필요에 맞게
...
hold off
axis equal
xlim([-5.5 5.5]); ylim([-2.5 7.5]); zlim([-4.5 8.5]);   % 필요에 맞게
xlabel('X'); ylabel('Y'); zlabel('Z');
...
grid on; axis equal;  hold off; view([1 1 1]);
%xlim([-0.01 0.01]); ylim([-0.01 0.01]); zlim([-0.01 0.01]);  

%%
%{
ndim  = cluster + 4;               % 사용할 고유벡터(축) 개수
Y_use     = Y(:, 1:ndim);                      % 앞에서부터 ndim축 사용 (필요시 조정)

% ---- GMM 클러스터링 ----
Kmix  = 6;                                 % 혼합 성분 수 (분리하고 싶은 군집 수)
