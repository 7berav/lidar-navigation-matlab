...
for e = eps_list
    ...
    for t = Thresh_list
            ...
            ransacPar.thresh  = t;
            ransacPar.mc.saveVarName = sprintf('MC_scores_w%02d_th%d_N%d_nT10', round(100*e),  round(100*t), Niter);
...
% 대상 조합 (필요시 수정)
w_list_pct = [0 02 05 10 ];     % w00, w02, w05, w10  → 4개
%w_list_pct = [20 30 35 40];     % w20, w30, w35, w40  → 4개
%w_list_pct = [35 40];
%L_list     = [2 4 7];        % L2, L4, L7          → 3개
t_list_pct = [35 40 45 50];
% 저장 폴더
outdir = fullfile(pwd, 'image_ISS/eth_10_2');
if ~exist(outdir,'dir'), mkdir(outdir); end

for wi = 1:numel(w_list_pct)
  for ti = 3:numel(t_list_pct)
    wpct = w_list_pct(wi);
    tpct = t_list_pct(ti);

    % 변수명/태그 구성
    mcvar = sprintf('MC_scores_w%02d_th%d_N%d_nT10', wpct, tpct, Niter);
