...
%양수 음수로 구분
margin = 0.00073;  % 예: 0.01 ~ 0.02 사이로 조정
...
pq   = [cluster+8 cluster+10];                 % <- 여기만 바꾸세요. [3 5]로 하면 3:5
idx3 = pq(1):pq(2);
...
if need_cut > 0
    % 모든 성분의 간선(원래 좌표로 환산)과 가중치 모으기
    all_edges = [];
    for i = 1:m
        Ti = T_cells{i};
        E  = Ti.Edges.EndNodes;      % 인덱스는 성분 내부
        W  = Ti.Edges.Weight;
    ...
    % 긴 간선부터 need_cut개 제거
    [~, ord] = sort(all_edges(:,3), 'descend');
    cut_list = all_edges(ord(1:min(need_cut, size(all_edges,1))), 1:2);
    ...
    for i = 1:m
        ...
        if any(mask_i)
            % 성분 내부 인덱스로 변환
            map = zeros(N,1); map(Vi) = 1:numel(Vi);
