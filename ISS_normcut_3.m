...
ev = E_uv_y(:,2); 
wplot = 1 ./ (1 + W_e_y); 
w01   = (wplot - min(wplot)) ./ max(eps, max(wplot)-min(wplot));  % [0,1] 정규화
...
%scatter3(Xc(:), Yc(:), Zc(:),4, [0.7 0.7 0.7], 'filled', ...
%    'MarkerFaceAlpha', 0.5, 'MarkerEdgeAlpha', 0.5);

colormap(parula(256)); caxis([0 1]);
for t = 1:size(E_uv_y,1)
    line( ...
      [Xc(eu(t)) Xc(ev(t))], ...
      [Yc(eu(t)) Yc(ev(t))], ...
      [Zc(eu(t)) Zc(ev(t))], ...
      'Color', interp1(linspace(0,1,256), parula(256), w01(t), 'linear', 'extrap'), ...
      'LineWidth', 0.5);
end
colorbar;
title(sprintf('MST (dims %d:%d)', idx3(1), idx3(end)));
view([1 1 1]); hold off
%% 분할 그리기
